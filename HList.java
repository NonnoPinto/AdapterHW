 ...
 * for elements in the list. Unlike sets, lists typically allow duplicate
 * elements. More formally, lists typically allow pairs of elements e1 and e2
 * such that e1.equals(e2), and they typically allow multiple null elements if
 * they allow null elements at all. It is not inconceivable that someone might
 * wish to implement a list that prohibits duplicates, by throwing runtime
 * exceptions when the user attempts to insert them, but we expect this usage to
 * be rare. The List interface places additional stipulations, beyond those
 ...
 * methods are also included here for convenience. The List interface provides
 * four methods for positional (indexed) access to list elements. Lists (like
 * Java arrays) are zero based. Note that these operations may execute in time
 * proportional to the index value for some implementations (the LinkedList
 * class, for example). Thus, iterating over the elements in a list is typically
 ...
 * interface provides. A method is provided to obtain a list iterator that
 * starts at a specified position in the list. The List interface provides two
 * methods to search for a specified object. From a performance standpoint,
 * these methods should be used with caution. In many implementations they will
 * perform costly linear searches. The List interface provides two methods to
 * efficiently insert and remove multiple elements at an arbitrary point in the
 * list. Note: While it is permissible for lists to contain themselves as
 * elements, extreme caution is advised: the equals and hashCode methods are no
 * longer well defined on a such a list. Some list implementations have
...
public interface HList extends HCollection {
     ...
     *                              null elements and this list does not support
     ...
     *                                       || index > size()).
     */
    public boolean addAll(int index, HCollection c) throws UnsupportedOperationException, ClassCastException,
     ...
     * = i.next(); hashCode = 31*hashCode + (obj==null ? 0 : obj.hashCode()); } This
     * ensures that list1.equals(list2) implies that
     ...
     * @throws UnsupportedOperationException if the set method is not supported by
     *                                       this list.
     * @throws ClassCastException            if the class of the specified element
     *                                       prevents it from being added to this
     *                                       list.
