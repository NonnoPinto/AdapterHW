 ...
 * abstraction. The Set interface places additional stipulations, beyond those
 * inherited from the Collection interface, on the contracts of all constructors
 * and on the contracts of the add, equals and hashCode methods. Declarations
 * for other inherited methods are also included here for convenience. (The
 * specifications accompanying these declarations have been tailored to the Set
 * interface, but they do not contain any additional stipulations.) The
 * additional stipulation on constructors is, not surprisingly, that all
 * constructors must create a set that contains no duplicate elements (as
 * defined above). Note: Great care must be exercised if mutable objects are
 * used as set elements. The behavior of a set is not specified if the value of
 * an object is changed in a manner that affects equals comparisons while the
 * object is an element in the set. A special case of this prohibition is that
 * it is not permissible for a set to contain itself as an element. Some set
 * implementations have restrictions on the elements that they may contain. For
 * example, some implementations prohibit null elements, and some have
 ...
 * will exhibit the former behavior and some will exhibit the latter. More
 * generally, attempting an operation on an ineligible element whose completion
 * would not result in the insertion of an ineligible element into the set may
...
public interface HSet extends HCollection {
     ...
     * @throws ArrayStoreException  the runtime type of a is not a supertype of the
     *                              runtime type of every element in this set.
     ...
     * set contains no element e such that (o==null ? e==null : o.equals(e)). If
     ...
     * constructors, this ensures that sets never contain duplicate elements. The
     * stipulation above does not imply that sets must accept all elements; sets may
     * refuse to add any particular element, including null, and throwing an
     * exception, as described in the specification for Collection.add. Individual
     * set implementations should clearly document any restrictions on the the
     ...
     *                                       this set does not support null
     *                                       elements.
     ...
     *                              null elements and this set does not support null
     *                              elements (optional).
     ...
     * is the union of the two sets. The behavior of this operation is unspecified
     ...
     *                                       this set are incompatible with the
     ...
     * member of this set is contained in the specified set). This definition
     ...
     * to be the sum of the hash codes of the elements in the set, where the
     * hashcode of a null element is defined to be zero. This ensures that
     * s1.equals(s2) implies that s1.hashCode()==s2.hashCode() for any two sets s1
     * and s2, as required by the general contract of the Object.hashCode method.
     *
