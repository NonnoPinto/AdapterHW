 ...
 * elements and others do not. Some are ordered and others unordered. The SDK
 ...
 * where maximum generality is desired. Bags or multisets (unordered collections
 * that may contain duplicate elements) should implement this interface
 * directly. All general-purpose Collection implementation classes (which
 * typically implement Collection indirectly through one of its subinterfaces)
 * should provide two "standard" constructors: a void (no arguments)
 * constructor, which creates an empty collection, and a constructor with a
 * single argument of type Collection, which creates a new collection with the
 * same elements as its argument. In effect, the latter constructor allows the
 ...
 * interfaces cannot contain constructors) but all of the general-purpose
 * Collection implementations in the Java platform libraries comply. The
 * "destructive" methods contained in this interface, that is, the methods that
 ...
 * unmodifiable collection may, but is not required to, throw the exception if
 * the collection to be added is empty. Some collection implementations have
...
public interface HCollection {
     ...
     * returned array will be "safe" in that no references to it are maintained by
     * this collection. (In other words, this method must allocate a new array even
     ...
     * size of this collection. If this collection fits in the specified array with
     * room to spare (i.e., the array has more elements than this collection), the
     * element in the array immediately following the end of the collection is set
     * to null. This is useful in determining the length of this collection only if
     * the caller knows that this collection does not contain any null elements.) If
     ...
     * order. Like the toArray method, this method acts as bridge between
     ...
     * circumstances, be used to save allocation costs Suppose l is a List known to
     * contain only strings. The following code can be used to dump the list into a
     * newly allocated array of String: String[] x = (String[]) v.toArray(new
     * String[0]); Note that toArray(new Object[0]) is identical in function to
     * toArray().
     * 
     ...
     * place limitations on what elements may be added to this collection. In
     * particular, some collections will refuse to add null elements, and others
     * will impose restrictions on the type of elements that may be added.
     * Collection classes should clearly specify in their documentation any
     * restrictions on what elements may be added. If a collection refuses to add a
    ...
    public boolean removeAll(HCollection c)
            throws ClassCastException, UnsupportedOperationException, NullPointerException;
     ...
     * (in other words, create a class that is a Collection but is not a Set or a
     * List) must exercise care if they choose to override the Object.equals. It is
     ...
     * neither the List nor Set interface must return false when this collection is
     * compared to any list or set. (By the same logic, it is not possible to write
