 ...
 * key can map to at most one value. This interface takes the place of the
 * Dictionary class, which was a totally abstract class rather than an
 * interface. The Map interface provides three collection views, which allow a
 ...
 * order; others, like the HashMap class, do not. Note: great care must be
 * exercised if mutable objects are used as map keys. The behavior of a map is
 * not specified if the value of an object is changed in a manner that affects
 * equals comparisons while the object is a key in the map. A special case of
 * this prohibition is that it is not permissible for a map to contain itself as
 * a key. While it is permissible for a map to contain itself as a value,
 * extreme caution is advised: the equals and hashCode methods are no longer
 * well defined on a such a map. All general-purpose map implementation classes
 * should provide two "standard" constructors: a void (no arguments) constructor
 * which creates an empty map, and a constructor with a single argument of type
 * Map, which creates a new map with the same key-value mappings as its
 * argument. In effect, the latter constructor allows the user to copy any map,
 * producing an equivalent map of the desired class. There is no way to enforce
 * this recommendation (as interfaces cannot contain constructors) but all of
 * the general-purpose map implementations in the SDK comply. The "destructive"
 * methods contained in this interface, that is, the methods that modify the map
 * on which they operate, are specified to throw UnsupportedOperationException
 * if this map does not support the operation. If this is the case, these
 * methods may, but are not required to, throw an UnsupportedOperationException
 * if the invocation would have no effect on the map. For example, invoking the
 * putAll(Map) method on an unmodifiable map may, but is not required to, throw
 * the exception if the map whose mappings are to be "superimposed" is empty.
...
public interface HMap {
     ...
     *                                       values, and the specified key or value
     *                                       is null.
     */
    public Object put(Object key, Object value)
            throws ClassCastException, NullPointerException, UnsupportedOperationException, IllegalArgumentException;
     ...
     * the map previously associated the key, or null if the map contained no
     * mapping for this key. (A null return can also indicate that the map
     ...
     * @throws NullPointerException          if the key is null and this map does
     *                                       not not permit null keys (optional).
     ...
     *                                       specified map prevents it from being
     *                                       stored in this map.
     * @throws NullPointerException          the specified map is null, or if this
     *                                       map does not permit null keys or
     ...
     * returned set is a Map.Entry. The set is backed by the map, so changes to the
